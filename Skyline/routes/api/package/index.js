...
router.get('/approval/:id', function (req, res) {
    ...
    SellVoucher.find({ _id:voucherId}).populate('elementConsumed.element').populate('package')
    .populate([{path: 'package',populate:{ path: 'Element',populate:{path:'element'}}}])
    .populate("sellTo")
    .exec(async function (err, voucher){
         if(err)
         ...
         var sellVoucher=voucher[0]
         sellVoucher.approved=true

          console.log("Sell",sellVoucher)
        SellVoucher.findByIdAndUpdate(voucherId, sellVoucher, {new: true}).
        then(data => {
           res.redirect('/api/package/approval');
         }).catch(error => {
