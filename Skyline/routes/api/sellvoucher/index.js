...
router.post('/sell_voucher_update/:id', (req, res, next) => { 
    ...
    if (errors) {
        ...
        res.json({ 'success': false, 'message': 'validation error', 'errors': errors });
          ...
          var id; 
          var arr=[]
          SellVoucher.findById(voucherId)
          .populate('package').
          exec(async function (err, vouc) {
              var sell=vouc
              if(err)
              console.log(err)
              else{
                for(var i=0;i<vouc.elementConsumed.length;i++)
                {
                    arr.push(vouc.elementConsumed[i]._doc)
                }

                var result;
                (vouc.elementConsumed).reduce(function(countMap, item) {
                    countMap[item.element] = ++countMap[item.element] || 1;
                    result=Object.entries(countMap).map(([key, value]) => ({key,value}));
                    return countMap;
                  }, {});
     
                sellLength=vouc.elementConsumed.length
                id=vouc._id
